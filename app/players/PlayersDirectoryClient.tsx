...
export interface Player extends DirectoryPlayer {
  id: string;
  title?: string;
...
const sanitizeAll = (entries: Player[]) =>
  entries.map((player) => ({ ...player, title: sanitizePlayerDisplayName(player.title) }));

export default function PlayersDirectoryClient({ initialPlayers }: { initialPlayers: Player[] | null }) {
  const [players, setPlayers] = useState<Player[]>(() => sanitizeAll(initialPlayers ?? []));
  ...
  useEffect(() => {
    // The server page passes the roster when Neon answered; fetch only as a fallback.
    if (initialPlayers !== null) return;
    async function fetchPlayers() {
      try {
        const response = await fetch("/api/players");
        if (!response.ok) throw new Error(`Players request failed: ${response.status}`);
        const data: PlayersResponse = await response.json();
        setPlayers(sanitizeAll(data.entries || []));
      } catch (fetchError) {
  ...
  return (
    <section className="base_paddings pb-20 pt-28 lg:pb-24 lg:pt-40">
      <div className="max_content center_aligned mx-auto">
        ...
        <div className="z-30 mt-8 rounded-[var(--radius)] border border-[var(--panel-line-strong)] bg-[color-mix(in_srgb,var(--ink)_92%,transparent)] p-3 shadow-2xl backdrop-blur-xl lg:sticky lg:top-[86px] lg:mt-12 lg:p-4">
